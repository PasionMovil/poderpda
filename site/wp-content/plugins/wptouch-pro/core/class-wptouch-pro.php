...
class WPtouchProThree {
	...
	function initialize() {
		...
		$bncid_settings->is_upgrading_to_four = 0;
	...
	function handle_admin_notices() {
		...
		$bncid_settings = wptouch_get_settings( 'bncid' );
		if ( $bncid_settings->upgrade_to_four_failed ) {
			echo '<div class="error"><p>' .
			sprintf( __( 'Preparation for upgrading to WPtouch 4.x failed. Please visit %sthis article%s for information on how to resolve this issue.', 'wptouch-pro' ), '<strong style="color: darkred"><a href="https://support.wptouch.com/support/solutions/articles/5000676842-upgrade-from-wptouch-pro-3-x-to-wptouch-pro-4-x-fails/">', '</a></strong>' ) .
			'</p></div>';	
		}
	...
	function check_for_version_upgrade() {
		$bncid_settings = wptouch_get_settings( 'bncid' );
		// Check for 4.0 Upgrade
		if ( strpos( WPTOUCH_BASE_NAME, "-pro-3" ) !== false ) {
			// We are running 3.x
			$failed = false;

			require_once( ABSPATH . '/wp-admin/includes/plugin.php' );
			
			$old_plugin_identifier = 'wptouch-pro-3/wptouch-pro-3.php';

			$new_wptouch_dir = WP_PLUGIN_DIR . '/wptouch-pro';
			if ( file_exists( $new_wptouch_dir ) ) {
				@rename( $new_wptouch_dir, $new_wptouch_dir . '-bak' );

				if ( !file_exists( $new_wptouch_dir . '-bak' ) ) {
					$failed = true;
				}
			} 
			...
			if ( !$failed ) {
				...
				$old_wptouch_dir = WP_PLUGIN_DIR . '/wptouch-pro-3';

				@rename( $old_wptouch_dir, $new_wptouch_dir );

				if ( !file_exists( $new_wptouch_dir ) ) {
					// Rename of directory failed
					$failed = true;
				} else {
					$old_wptouch_file = WP_PLUGIN_DIR . '/wptouch-pro/wptouch-pro-3.php';
					$new_wptouch_file = WP_PLUGIN_DIR . '/wptouch-pro/wptouch-pro.php';

					@rename( $old_wptouch_file, $new_wptouch_file );

					if ( file_exists( $new_wptouch_file ) ) {
						deactivate_plugins( $old_plugin_identifier );
						...
						@rename( $new_wptouch_dir, $old_wptouch_dir );
						$failed = true;
					}
				}	
			}

			$bncid_settings->is_upgrading_to_four = true;
			$bncid_settings->upgrade_to_four_failed = $failed;
			...
			// Successfully changed the train tracks
			if ( $bncid_settings->is_upgrading_to_four  ) {
				$bncid_settings->is_upgrading_to_four = false;
				$bncid_settings->upgrade_to_four_failed = false;
	...
	function admin_handle_init() {
		...
		$this->check_for_version_upgrade();
